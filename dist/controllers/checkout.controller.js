...
exports.cancelShipmozoOrder = exports.createShipmozoOrder = exports.calculateDeliveryChargeWithoutCart2 = exports.calculateDeliveryCharge2 = exports.getExistingAWB = exports.cancelShiprocketOrder = exports.calculateDeliveryChargeWithoutCart = exports.createDirectCodOrder = exports.createCodOrder = exports.calculateDeliveryCharge = exports.createShiprocketOrder = exports.verifyDirectPayment = exports.verifyPayment = exports.createDirectCheckoutSession = exports.createCheckoutSession = void 0;
...
const createDirectCheckoutSession = async (req, res) => {
    try {
        if (!razorpay) {
            razorpay = initializeRazorpay();
            if (!razorpay) {
...
exports.createDirectCheckoutSession = createDirectCheckoutSession;
const verifyPayment = async (req, res) => {
    try {
        ...
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, deliveryRate, shippingAddress, promoCode, promoCodeDiscount } = req.body;
        const key_secret = process.env.RAZORPAY_KEY_SECRET;
        if (!key_secret) {
            res.status(500).json({ message: "Payment service not configured" });
            return;
...
const verifyDirectPayment = async (req, res) => {
    try {
        ...
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, deliveryRate, shippingAddress, total, price, product, quantity, variant, dimensions, promoCode, promoCodeDiscount } = req.body;
        const key_secret = process.env.RAZORPAY_KEY_SECRET;
        if (!key_secret) {
            res.status(500).json({ message: "Payment service not configured" });
            return;
...
exports.verifyDirectPayment = verifyDirectPayment;
// SHIPROCKET CONFIG
...
const createCodOrder = async (req, res) => {
    try {
        ...
        if (promoCode) {
            await promo_model_1.PromoCode.updateOne({ code: promoCode }, { $inc: { usedCount: 1 } });
        }
...
const createDirectCodOrder = async (req, res) => {
    try {
        const userId = req.params.id;
        const { deliveryRate, courierId, shippingAddress, price, product, quantity, variant, dimensions, promoCode, promoCodeDiscount } = req.body;
        const productOrdered = await product_model_1.Product.findById(product);
        if (!productOrdered) {
...
exports.createDirectCodOrder = createDirectCodOrder;
const calculateDeliveryChargeWithoutCart = async (req, res) => {
    try {
...
exports.calculateDeliveryChargeWithoutCart = calculateDeliveryChargeWithoutCart;
const cancelShiprocketOrder = async (req, res) => {
    try {
        ...
        const token = await getShiprocketToken();
        const response = await axios_1.default.post(`${SHIPROCKET_API_BASE}/orders/cancel`, {
        ...
        if (response.data.status_code === 200 || response.data.status == 200) {
            await order_model_1.Order.updateOne({ shiprocketOrderId }, {
                status: 'cancelled',
...
const getExistingAWB = async (req, res) => {
    try {
        ...
        const token = await getShiprocketToken();
        const response = await axios_1.default.get(`${SHIPROCKET_API_BASE}/orders/show/${order_id}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
exports.getExistingAWB = getExistingAWB;
// SHIPMOZO CONFIG
const SHIPMOZO_API_BASE = "https://shipping-api.com/app/api/v1";
const calculateDeliveryCharge2 = async (req, res) => {
    try {
        const { userId, destinationPincode, cod } = req.body;
        const cart = await cart_model_1.Cart.findOne({ user: userId }).populate("items.product");
        if (!cart || cart.items.length === 0) {
        ...
        const response = await axios_1.default.post(`${SHIPMOZO_API_BASE}/rate-calculator`, params, {
            headers: {
        ...
        // You can return all options or pick the cheapest
        const cheapest = shippingOptions.reduce((a, b) => a.total_charges < b.total_charges ? a : b);
        res.json({
...
exports.calculateDeliveryCharge2 = calculateDeliveryCharge2;
const calculateDeliveryChargeWithoutCart2 = async (req, res) => {
    try {
...
exports.calculateDeliveryChargeWithoutCart2 = calculateDeliveryChargeWithoutCart2;
const createShipmozoOrder = async (req, res) => {
    try {
        const { orderId, paymentMethod, courierId } = req.body;
        const order = await order_model_1.Order.findById(orderId)
            .populate("items.product")
        ...
        const response = await axios_1.default.post(`${SHIPMOZO_API_BASE}/push-order`, shipmozoOrder, {
            headers: {
                "public-key": process.env.SHIPMOZO_PUBLIC_KEY || "",
                "private-key": process.env.SHIPMOZO_PRIVATE_KEY || ""
            },
        ...
        try {
            const pickupRequestCall = await axios_1.default.post(`${SHIPMOZO_API_BASE}/schedule-pickup`, {
                order_id: shippingId,
            }, {
                headers: {
                    "public-key": process.env.SHIPMOZO_PUBLIC_KEY || "",
...
exports.createShipmozoOrder = createShipmozoOrder;
const cancelShipmozoOrder = async (req, res) => {
    try {
        const { shipmozoOrderId, awbNumber, reason } = req.body;
        if (!shipmozoOrderId) {
            return res.status(400).json({ message: "Order ID is required" });
        }
        const response = await axios_1.default.post(`${SHIPMOZO_API_BASE}/cancel-order`, {
        ...
        if (response.data.status_code === 200 || response.data.status == 200) {
            await order_model_1.Order.updateOne({ shippingOrderId: shipmozoOrderId }, {
                status: 'cancelled',
                cancellationReason: reason
            });
            return res.status(200).json({
...
exports.cancelShipmozoOrder = cancelShipmozoOrder;
